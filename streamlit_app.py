...
def main():
    ...
    with col1:
        ...
        if selected_example != "None":
            example = next(
                (ex for ex in examples.values() if ex["name"] == selected_example)
            )
            user_message = st.text_area(
                "User Message (Required)", value=example["user_message"], height=100
            )
            system_prompt = st.text_area(
                "System Prompt (Required)", value=example["system_prompt"], height=100
            )
            chat_context = st.text_area(
                "Prior Chat Context (Optional)",
                value=example.get("chat_context", ""),
                height=100,
            )
        else:
